							...
							<table id="tb" class="table table-striped table-bordered">
								<thead>
									<tr>
										...
										<th>Borrar</th>
										<?php } ?>
										<th class="col-1 text-center">Folio</th>
										<th>Hora</th>
										<th>Fecha</th>
										<th>Cuenta</th>
									...
									<tr>
										<td class="text-secondary"><?= esc($pago['pago_id']) ?></td>
											...
											<td class="text-center">
												<a href="/pagos/borrar_pago_tienda/<?= esc($pago['pago_id']) ?>/<?php echo $date ?>" class="btn btn-sm btn-danger" onclick="return confirm('¿Estás seguro de Borrar el pago?')"><i class="fa-solid fa-trash-can"></i></a>
											</td>
											<?php } ?>
										<td><?= esc($pago['folio']) ?></td>
										<td><small><?= esc(date('h:i A', strtotime($pago['hora']))) ?></small></td>
										<td><small><?= esc(date('d/m/y', strtotime($pago['fecha_pago']))) ?></small></td>
										<td><?= esc($pago['cta']) ?></td>
										<td class="text-center"><button class="btn btn-secondary btn-sm" data-bs-toggle="modal" data-bs-target="#recibo" data-recibo="<?= esc($pago['recibo']) ?>" data-id="<?= esc($pago['pago_id']) ?>" data-folio="<?= esc($pago['folio']) ?>"><i class="fa-solid fa-receipt"></i></button></td>
										<td class="text-center">
...
$('#datepk').change(function(event) {
	...
	window.location.href = "/pagos/fecha_tienda/"+fSelect
});
...
$('#recibo').off('show.bs.modal').on('show.bs.modal', function (event) {
	...
	$('#recibo-ver').attr('src', 'https://centrodemayoreocdmx.com.mx/pagos/'+recibo);

	$('#clienteNom').html(cliente)
	$('#folioRev').html(folio)
	$('#totalRev').html(total)
    $('#lmVerif').attr('href', '/pagos/upd_verifPTienda/'+id+'/'+'<?php echo $date; ?>');
    $('#lmImpreso').attr('href', '/pagos/upd_impreso_tienda/'+id+'/'+'<?php echo $date; ?>');
    $('#lmGuia').attr('href', '/pagos/upd_guia_tienda/'+id+'/'+'<?php echo $date; ?>');
    $('#lmAtencion').attr('href', '/pagos/upd_atencion_tienda/'+id+'/'+'<?php echo $date; ?>');
    $('#lmQatencion').attr('href', '/pagos/upd_atencion_q_tienda/'+id+'/'+'<?php echo $date; ?>');
})
